...
class App extends Component<Props> {
  ...
  render() {
    ...
    var redirectToAuth = false;
    if (!redirectToAuth) {
      var matchedRoutes = routes.filter(r => r.path === location.pathname);
      if (matchedRoutes && matchedRoutes.length > 0) {
        if (!isLoggedIn && matchedRoutes[0].tag === RouteAuth) {
          redirectToAuth = true;
        }
      }
    }
    if (location.pathname === "/auth") redirectToAuth = false;
...
function mapDispatchToProps(dispatch) {
  ...
  return {
    ...
    checkAuthLogic(path: string, isLoggedIn: boolean) {
      console.log("APP check auth logic");
      const authPath = "/auth";
      const homePath = "/";
