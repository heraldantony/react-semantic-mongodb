...
class SignupComponent extends Component<Props, State> {
	render () {
        ...
        */
		...
		const { error, handleSubmit, signup, invalid, submitting, submitSucceeded } = this.props
		return (
			<div>
				...
				{submitSucceeded ? (<Message positive>
					<Message.Header>Your user registration was successful</Message.Header>
					<p>Please <Link to="/auth">login here</Link>.</p>
				</Message>) : (
					<Message attached='bottom' warning>
						<Icon name='help' />
              Already signed up?&nbsp;<Link to="/auth">Login here</Link>&nbsp;instead.
					</Message>)
				}
			</div>
		)
