...
import { searchJob, deleteJob } from 'common/actions/job'
									...
									<Table.Row key={'job_' + idx}>
										...
										<Table.Cell>
											...
											<ConfirmationDialog
												title={'Delete Job'}
												content={
													'Are you sure you want to delete Job with Id ' +
                          job['_id'] +
                          ' and jobTitle ' +
                          job['jobTitle']
												}
												triggerButtonContent={<Icon name="trash" />}
												okFunction={() => deleteEntity(job)}
											/>
										</Table.Cell>
									</Table.Row>
								)
...
const mapDispatchToProps = dispatch => ({
	...
	deleteEntity (job) {
		...
		return dispatch(deleteJob(job._id))
	}
})
const withConnect = connect(mapStateToProps, mapDispatchToProps)

export default reduxForm({ form: 'JOB_SEARCH_FORM' })(
	compose(withConnect)(JobSearch)
)
