/*
 *  Test sagas for deleting Department 
 *
*/
...
import { doDeleteDepartment, deleteDepartment } from "..";

/* eslint-disable redux-saga/yield-effects */
describe("doDeleteDepartment Saga", () => {
  let doDeleteDepartmentGenerator;

  beforeEach(() => {
    doDeleteDepartmentGenerator = doDeleteDepartment();
  });
  afterEach(() => {});
  ...
  it("should delete Department ", () => {
    ...
    return (
      expectSaga(doDeleteDepartment, action)
        .provide([[call(departmentDeleteAPI, action.payload), fakeResult]])
        //assert that saga will eventually yield deleteDepartmentSuccess action
        .put(deleteDepartmentSuccess(fakeResult.data))
        .run()
    );
  });
  it("should fail to delete Department with message", () => {
    let fakeDepartment = {
      _id: "5b254369063db83598df2cfe",
      departmentName: "Optimization",
      location: {
    ...
    return (
      expectSaga(doDeleteDepartment, action)
        .provide([[call(departmentDeleteAPI, action.payload), fakeResult]])
        //assert that saga will eventually yield deleteDepartmentFail action with error message
        .put(deleteDepartmentFail(fakeResult.data.message))
        .run()
        .catch(error => {
  ...
  it("should handle reducer and store state", () => {
    ...
    let departmentId = fakeDepartment._id;
    let fakeResult = { ok: true, data: fakeDepartment };
    let finalState = { ...initialState, department: {} };
    let action = { payload: departmentId };
    return expectSaga(doDeleteDepartment, action)
      .withReducer(departmentReducer)
      .provide([[call(departmentDeleteAPI, action.payload), fakeResult]])
      .hasFinalState(finalState)
      .dispatch(
        deleteDepartmentAction(action.payload, action.form, action.promise)
      )
      .run();
  });
});
